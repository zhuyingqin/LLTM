...
| C7.（2026-07-01 新增，**首次真实数字**）冻结 Qwen3.5-0.8B 经 adapter token 接入 LAFR，能在 MG 合成数据的结构化问答上实现真接地（correct≫shuffled≈no_token），验证"统一入口+内部路由"机制本身可行 | `results/mg_llm_adapter/mg_llm_adapter_report.json`：总体 gap=+0.256；分任务 conf(+0.542)/lead(+0.215) 接地成立，**cp(−0.107) 未接地且方向反**（详见 [mg_llm_track/README.md](mg_llm_track/README.md)） | **部分支持**——机制在 2/3 任务类型上有真实证据，cp 任务异常需先诊断（怀疑是注入幅度按 CARE 尺度校准、不适配标准化后的 MG 数据）才能作为可靠证据使用；不能替代 CARE 真实数据的 C1/C2 | §4（作为 CARE 重跑前的可行性佐证 + 诚实报告 cp 异常）, §6（局限） |
