...
| **MG 简化验证线**：MG 合成数据 → LAFR（冻结）→ adapter 对齐 token → 冻结 **Qwen3.5-0.8B** → 结构化问答（lead/conf/cp 三任务），correct/shuffled/no_token 三重接地对照 | [experiments/mg_llm_adapter.py](experiments/mg_llm_adapter.py)（2026-07-01 新增脚本，详细结果解读见 [mg_llm_track/README.md](mg_llm_track/README.md)） | **有**：`results/mg_llm_adapter/mg_llm_adapter_report.json`（跑通，耗时 12679s≈3.5h/单卡3060） | **真实落盘结果**（2026-07-01）：总体 correct=0.750 ≫ shuffled=0.494 ≈ no_token=0.461（gap+0.256，接地成立）。**但按任务拆分是 2/3 grounded、1 个异常**：conf 任务 gap=+0.542（极强）、lead 任务 gap=+0.215（成立）、**cp（变化点）任务 gap=−0.107（未接地且方向反了，shuffled 比 correct 还准）**——这是当前最值得深挖的具体异常，不是笼统的"部分成功"。这是 adapter+冻结LLM接地协议**首次**在真实模型上拿到数字（CARE 线至今仍是"协议已写、从未跑过"）；可作为 CARE 线（C1/C2）真正重跑前的机制可行性佐证，但不能替代 CARE 真实数据结论 |

**结论：现在手上真正"可引用"的数字有三条线——AnomLLM 合成基准（主结果）、Mackey-Glass forecast head 功能性验证（辅助证据）、MG 简化验证线的接地结果（部分成立，2/3 任务，含一个需诊断的异常）。** CARE 真实数据这条线（问答接地 + 机制评估）代码质量很高、协议设计得也很讲究（三重接地对照、CoT 公平性检查这些都是很扎实的实验设计），但数字全部缺失，必须重跑才能写进 Results。这不是否定这条线，而是提醒：**在启动 `/paper-write` 之前，必须先把 CARE 这条线的实验重新跑一遍并把 json 保存下来**（其余数据/权重原本大概率在 `C:\杂物\LLM-NN\` 这个更大的工作区里，`anomllm-compare/*.json` 里的 `config.data_root`/`model` 字段还留着这个路径，值得先去那边找一下有没有现成的 `results/lafr_eval_full_fixed/`、`results/llm_adapter_full_qwen3/` 之类的产出，能省一次重跑）。
...
- **✅ 2026-07-01 补充进展**：接口 A（token 接地）在 Qwen3.5-0.8B 上的 MG 简化验证已跑完，结果见 §0 证据盘点新增行 + [mg_llm_track/README.md](mg_llm_track/README.md)——总体接地成立（gap+0.256），但 2/3 任务而非 3/3，change-point 任务方向异常。接口 B 的 0.8B smoke test **仍未做**，这一步和 CARE 线重跑一起才需要触发。
...
| C7.（2026-07-01 新增，**首次真实数字**）冻结 Qwen3.5-0.8B 经 adapter token 接入 LAFR，能在 MG 合成数据的结构化问答上实现真接地（correct≫shuffled≈no_token），验证"统一入口+内部路由"机制本身可行 | 两轮独立协议的真实结果：① logit 打分（`results/mg_llm_adapter/mg_llm_adapter_report.json`）总体 gap=+0.256，conf(+0.542)/lead(+0.215) 接地、cp(−0.107) 反常；② **自由生成解释 + 事实核查**（`results/mg_llm_explain/mg_llm_explain_report.json`，2026-07-01 第二轮）：LLM 生成完整自然语言解释，事实准确率 conf 0.800(gap+0.229)/lead 0.704(gap+0.296) 接地，**cp 0.429(gap=0.000，模型恒答 Yes)**；且正确 token 显著抑制数值幻觉（tau 误差 1.05 patch vs no_token 条件幻觉出 10^30 量级数字）。详见 [mg_llm_track/README.md](mg_llm_track/README.md) | **部分支持且跨协议可复现**——conf/lead 两类结构在打分和自由生成两种输出协议下都接地；**cp 在两种协议下都不接地**，基本排除评估方式问题，指向 change-point 信号未通过 event-token 通路传入 LLM（首要怀疑注入幅度按 CARE 尺度校准、不适配标准化 MG）；这是可写进论文分析节的可复现现象。不能替代 CARE 真实数据的 C1/C2 | §4（可行性佐证 + cp 异常分析）, §6（局限） |
...
- [x] ~~MG 简化验证线：接口 A 在 Qwen3.5-0.8B 上跑通~~ **已完成（2026-07-01）**——总体接地成立（gap+0.256），但 conf/lead 接地、**cp 未接地且方向异常（gap−0.107）**，详见 C7 与 [mg_llm_track/README.md](mg_llm_track/README.md)
- [x] **MG 解释版（用户核心诉求：LLM 输出自然语言解释而非被打分）已完成（2026-07-01 第二轮，`mg_llm_explain.py`）**——LLM 自由生成解释、按注入 GT 做事实核查：conf/lead 解释接地（gap+0.229/+0.296），cp 依旧不接地（恒答 Yes）；cp 失败跨两种协议复现，升级为需要专门诊断的可复现现象
- [ ] **诊断 cp 任务异常（新增，优先级高于恢复 CARE 重跑）**：怀疑 `make_changepoint` 的固定 `gain=1.8/shift=1.5` 是按 CARE 真实尺度校准的，套到标准化后的 MG 数据上可能不匹配；先试改成按每窗口标准差比例注入，重跑 `mg_llm_adapter.py` 看 cp 的 gap 会不会转正
- [ ] MG 线补至少 2-3 个不同 seed，报 mean±std（当前只有一次运行，不知道 conf/lead 的接地强度有多稳、cp 的负 gap 是否是噪声）
- [ ] **恢复大论文计划时再做**：重跑 CARE track 三个脚本（`lafr_eval.py` / `llm_adapter.py` / `llm_compare.py`），**骨干换 Qwen3.5-0.8B**，把 `--output-dir` 指向的 json 真正落盘；`lag_readout_probe.py` 加一个落盘出口再重跑
